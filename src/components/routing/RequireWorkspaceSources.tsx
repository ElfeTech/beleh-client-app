...
import { WorkspacePageSkeleton } from '../workspace/WorkspaceLoadingSkeletons';
...
export function RequireWorkspaceSources({ children }: Readonly<{ children: ReactNode }>) {
  ...
  const sourcesReadyForWorkspace = Boolean(id) && workspaceContext?.workspace.id === id && !loading;
