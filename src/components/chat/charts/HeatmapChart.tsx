...
export const HeatmapChart: React.FC<HeatmapChartProps> = ({
  ...
  try {
    ...
    const minValue = allColorValues.length > 0 ? Math.min(...allColorValues) : 0;
    const maxValue = allColorValues.length > 0 ? Math.max(...allColorValues) : 1;
    ...
    console.error('HeatmapChart rendering error:', error, { data, xField, yField, colorField });
    return (
      <div className="chart-error">
        <p>Unable to render heatmap: {error instanceof Error ? error.message : 'Unknown error'}</p>
      </div>
    );
  }
};
