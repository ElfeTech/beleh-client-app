...
function HorizontalBarPlot({
  ...
  if (!formatted) {
    return <p className="text-sm text-[color:var(--text-muted)]">Unable to render plot for this encoding.</p>;
  }
...
export function InteractivePlotView({
  ...
  const xField =
    visualization.encoding?.x?.field || visualization.dimensions?.x || 'category';
  const yField =
    visualization.encoding?.y?.field || visualization.dimensions?.y || 'value';
  const subtitle = `Auto-fitted to ${xField} vs ${yField}`;

  const useHorizontalBars = chartType === 'bar';
        ...
        <div className="interactive-plot__chips" role="group" aria-label="Chart type">
          {plotTypes.map((t) => (
            <button
              key={t}
              type="button"
              className={cn('interactive-plot__chip', chartType === t && 'interactive-plot__chip--active')}
