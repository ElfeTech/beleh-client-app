...
interface DatasourceModalProps {
    ...
    datasourceId?: string; // Required for edit mode
...
export function DatasourceModal({ mode, workspaceId, datasourceId, initialName = '', onClose, onSuccess }: DatasourceModalProps) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedFile) {
            ...
            // Auto-populate name from filename (remove extension) only in add mode
            if (mode === 'add') {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validation: add mode requires file, edit mode doesn't
        if (mode === 'add' && !file) return;
        if (mode === 'add' && !workspaceId) return;
        if (mode === 'edit' && !datasourceId) return;
        ...
        try {
            ...
            let dataset: DataSourceResponse;

            if (mode === 'add') {
                // Create new datasource
                dataset = await apiClient.createDatasource(token, workspaceId!, file!, name);
                ...
                if (file) {
                    // User uploaded a new file - override
                    dataset = await apiClient.overrideDatasource(token, datasourceId!, file, name.trim());

                    setUploadStatus(dataset.status);
                    setProgress(25);

                    // Start polling for status updates when file is replaced
                    pollIntervalRef.current = setInterval(() => {
                        pollDatasetStatus(dataset.id);
                    }, 2000);
                    ...
                    // User only renamed - no polling needed
                    dataset = await apiClient.renameDatasource(token, datasourceId!, name.trim());
    ...
    const getStatusText = () => {
        switch (uploadStatus) {
            case 'IDLE':
                return '';
            case 'UPLOADING':
                return mode === 'edit' && !file ? 'Saving changes...' : 'Uploading file...';
            case 'PENDING':
                return 'Queued for processing...';
            case 'PROCESSING':
                return 'Processing and analyzing data...';
            case 'READY':
                return mode === 'edit' && !file ? 'Changes saved!' : 'Dataset ready!';
            case 'FAILED':
    ...
    const modalContent = (
        <div className="modal-backdrop" onClick={!isProcessing ? handleClose : undefined} style={{ zIndex: 10001 }}>
            <div className="modal-container" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                    <h2>{mode === 'add' ? 'Add New Dataset' : 'Edit Dataset'}</h2>
                    {!isProcessing && (
                ...
                <form onSubmit={handleSubmit} className="modal-form">
                    <div className="form-group">
                        <label>
                            {mode === 'add' ? 'Select File' : 'Replace Dataset File'}
                            {mode === 'edit' && <span style={{ color: 'var(--gray-500)', fontWeight: 400, marginLeft: '4px' }}>(Optional)</span>}
                        </label>
                        <div
                        ...
                        >
                            ...
                            {file ? (
                                ...
                                <div className="upload-placeholder">
                                    ...
                                    <span>Click to upload CSV, Excel, or JSON</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {uploadStatus !== 'IDLE' && (
                        <div className="upload-progress-container">
                            ...
                            {/* Only show stages for add mode or edit mode with file upload */}
                            {(mode === 'add' || file) && (
                    ...
                    <div className="modal-actions">
                        ...
                        <button
                            ...
                            disabled={(mode === 'add' && !file) || !name.trim() || isProcessing || isComplete}
                        >
                            {isProcessing
                                ? (file ? 'Processing...' : 'Saving...')
                                : isComplete
                                    ? 'Completed'
                                    : mode === 'add'
                                        ? 'Upload Dataset'
