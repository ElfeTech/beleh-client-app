...
export const HeaderSelection: React.FC<HeaderSelectionProps> = ({ sheets, onSubmit, onBack }) => {
  ...
  return (
    <div className="header-selection-container">
            ...
            >
              ...
              >
                <div className="sheet-meta">
                  <span
                    className={`status-dot ${isHeaderSelected(sheet.name) ? 'done' : 'pending'}`}
                  />
                  <span className="sheet-name">{sheet.name}</span>
                  {isHeaderSelected(sheet.name) && (
                    <span className="selected-row-info">
                      Row {selectedHeaders[sheet.name] + 1} selected
                    </span>
                  )}
                </div>
                <svg
              ...
              {expandedSheet === sheet.name && (
                <div className="accordion-content">
                  ...
                  {sheet.preview_rows && sheet.preview_rows.length > 0 ? (
                    ...
                    <div className="no-preview-msg">No data preview available for this sheet.</div>
                  )}
                </div>
              )}
            </div>
      ...
      <div className="header-selection-footer">
        <button
          type="button"
          className="btn-gradient-primary submit-btn"
